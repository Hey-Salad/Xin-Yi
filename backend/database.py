...
def init_database():
    ...
    # 创建出入库记录表 | Create inventory records table
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS inventory_records (
            ...
            operator TEXT DEFAULT '系统',  -- System
            reason TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
def generate_mock_data():
    ...
    # 生成出入库记录（近7天）| Generate inventory records (last 7 days)
    material_ids = [row[0] for row in cursor.execute('SELECT id FROM materials').fetchall()]

    reasons_in = ['采购入库', '生产完工入库', '退货入库', '调拨入库']  # Purchase, Production complete, Return, Transfer
    reasons_out = ['生产领料', '销售出库', '研发领用', '调拨出库', '返修出库']  # Production picking, Sales, R&D, Transfer, Repair
    operators = ['张三', '李四', '王五', '赵六', '系统']  # Zhang San, Li Si, Wang Wu, Zhao Liu, System

    # 生成过去7天的记录 | Generate records for the past 7 days
    for day_offset in range(7, 0, -1):
        record_date = datetime.now() - timedelta(days=day_offset)
        # 每天5-15条记录 | 5-15 records per day
        ...
        for _ in range(num_records):
            ...
            # 随机时间（当天的某个时间）| Random time (some time during the day)
    ...
    # 生成今天的记录（更多一些）| Generate today's records (more records)
    today = datetime.now()
